...
class ConcurrencyLimiter:
    ...
    def __init__(
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="ai_worker"
        )
    ...
    def _adapt_limits(self):
        ...
        if self.resource_guard.is_under_pressure():
            ...
            if self.semaphore._value > reduced_limit:
                logger.warning(
                    f"Adaptive Scaling: Reducing task concurrency to {reduced_limit} due to pressure."
                )
    ...
    async def run_async(
        ...
        if self.resource_guard.should_reject_task(priority):
            logger.error(
                f"Load Shedding: Rejecting async task {func.__name__} due to resource exhaustion."
            )
    ...
    async def run_in_thread(
        ...
        if self.resource_guard.should_reject_task(priority):
            logger.error(
                f"Load Shedding: Rejecting CPU task {func.__name__} due to resource exhaustion."
            )
