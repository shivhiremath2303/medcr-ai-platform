    ...
    async def call(self, func: Callable[..., T], *args: Any, **kwargs: Any) -> T:
        if self.state == CircuitState.OPEN:
            if time.time() - self.last_failure_time > self.recovery_timeout:
                ...
                if self.metrics:
                    self.metrics.increment_counter("resilience_circuit_tripped_total", {"name": self.name})
                raise RuntimeError(f"Circuit Breaker [{self.name}] is OPEN. Dependency is unreachable.")
        ...
        try:
            ...
            if self.failure_count >= self.failure_threshold:
                if self.state != CircuitState.OPEN:
                    logger.error(f"Circuit Breaker [{self.name}] TRIPPED (OPEN). Failures: {self.failure_count}")
                    self.state = CircuitState.OPEN

            if self.metrics:
                self.metrics.increment_counter("resilience_dependency_failure_total", {"name": self.name, "error": type(e).__name__})
