...
def validate_config(settings: Settings) -> None:
    ...
    if settings.environment == "production":
        ...
        if not settings.cors_allowed_origins or settings.cors_allowed_origins == ["*"]:
            errors.append("CORS_ALLOWED_ORIGINS must be explicitly set in production (no wildcards).")

        if settings.debug:
            errors.append("DEBUG must be false in production environment.")

        if "*" in settings.cors_allowed_methods:
            errors.append("CORS_ALLOWED_METHODS must not contain wildcards in production.")

        if "*" in settings.cors_allowed_headers:
            errors.append("CORS_ALLOWED_HEADERS must not contain wildcards in production.")
    ...
    for attr in ["rate_limit_auth_requests", "rate_limit_upload_requests", "rate_limit_rag_requests", "rate_limit_general_requests"]:
