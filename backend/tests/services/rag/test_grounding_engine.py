...
def test_calculate_grounding_score_with_citations(engine):
    ...
    score = engine.calculate_grounding_score(
        evidence, citations, SufficiencyLevel.SUFFICIENT, []
    )
    assert score > 0.8
...
def test_calculate_grounding_score_contradiction_penalty(engine):
    ...
    score_no_conflict = engine.calculate_grounding_score(
        evidence, ["[Evidence 1]"], SufficiencyLevel.SUFFICIENT, []
    )
    score_with_conflict = engine.calculate_grounding_score(
        evidence, ["[Evidence 1]"], SufficiencyLevel.SUFFICIENT, ["Conflict info"]
    )
